...
class Localpart(nn.Module):
    ...
    def get_image_pred_attn_global_version2(self, model_clip,model_attn,return_full=False, return_logit=False):
        ...
        img_feat_loacl_mappling = model_clip.image_features_local
        img_feat = model_clip.image_features_global  # 1*1024
        llm_local_Key =model_clip.llm_local_text
        memorized_image_feat = model_clip.fixed_global_feat
        memorized_image_feat_V = memorized_image_feat
        N = memorized_image_feat_V.shape[1]
        if N < 17:
            repeat_times = 17 - N
            random_indices = torch.randint(0, N, (repeat_times,), device=memorized_image_feat_V.device)
            random_image_feats = memorized_image_feat_V[:, random_indices, :]  # (102, repeat_times, 512)
            memorized_image_feat_V = torch.cat([memorized_image_feat_V, random_image_feats], dim=1)  # (102, 17, 512)
        else:
            memorized_image_feat_V = memorized_image_feat_V
        batch_size=img_feat.shape[0]
        ##############################################################
        with torch.no_grad():
            attn_weight_list = model_attn.attn_weight_list

            # 3. 生成注意力掩码
            attn_weight=attn_weight*attn_cnt
        logits_list = []

        llm_local_Key=llm_local_Key+self.local_llm_bias_key.unsqueeze(1)

        for i in range(batch_size):

            logits = torch.matmul(logits,attn_weight_i)
        logits_tensor = torch.stack(logits_list, dim=0)

        logits_tensor = torch.squeeze(logits_tensor, dim=3)
        logits_tensor=logits_tensor.permute(1,2,0)
        similarity_matrix= torch.exp(-self.beta * (-logits_tensor + 1))
        memorized_image_feat_V=memorized_image_feat_V+self.local_llm_bias_value.unsqueeze(1)
                ...
                for lr in lr_list:
                    for epoch in epoch_list:
                        ...
                        with torch.no_grad():
                            ...
                            model_clip.get_simple_text_features(datasets,16)
                        if args.n_shot:
                            if args.n_augview == 0:
...
def direct_inference(val_loader, model_clip,model_attn, args):
    ...
    if args.n_shot and args.ft:
        ...
        scheduler_b= torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_b, epoch * len(train_loader_ft), eta_min=1e-7)
        ...
        for train_idx in range(epoch): ## for each epoch.
            ...
            for i, (images, target) in enumerate(train_loader_ft):
                ...
                with torch.no_grad():
                    image_features_global, image_features_local = model_clip.get_image_features(images) ##B*D, B*L*D
                    model_attn.get_image_attn(images)

                fewshot_local_logit= localpart.get_image_pred_attn_global_version2(model_clip,model_attn, return_full=True, return_logit=True)


                loss_b=Loss(fewshot_local_logit, target)

                if args.position == 'output' or args.position == 'all':
                    text_logit = localpart.get_text_prediction(model_clip, return_full=True, return_logit=True)
                    localpart.init_pred = text_logit  ## use it for local few shot.
                    loss_b += Loss(text_logit, target)
                else:
                    with torch.no_grad():
                        text_logit = localpart.get_text_prediction(model_clip, return_full=True, return_logit=True)
                        localpart.init_pred = text_logit           ## use it for local few shot.
                ...
                loss_local_list.append(loss_b.item())
            ...
            print('LR: {:.6f}, text:{:},  local: {:} All:{:}), Loss_local: {:.4f}'.format(current_lr, correct_samples ,  correct_samples_local, all_samples, sum(loss_local_list) / len(loss_local_list),))


    # torch.save(localpart.state_dict(),'DTD_16shot.pth')
...
################################################################################################################################################
        ...
        if (i+1) % 1000 == 0:
            ...
            print("clip_logits is：" + str(acc))

            acc,_= accuracy(fewshot_local, label, topk=(1, 5))
            print("pred_fewshot_local is：" + str(acc))
    ...
    pred_text = torch.cat(pred_vanilla, dim=0)
    # pred_global_cache = torch.cat(pred_global, dim=0)
    # fewshot_global = torch.cat(pred_fewshot_global, dim=0)
    fewshot_local = torch.cat(pred_fewshot_local, dim=0)
    # pred_local_cache=torch.cat(pred_local, dim=0)
    label = torch.cat(labels, dim=0)
    ...
    if weight_search:
        ...
        # print(f"Searched Acc: {best_acc:.2f} with beta1 {best_beta1:.3f}, dynamic {best_beta2:.3f} and static {best_beta3:.3f},local_best {best_beta4:.3f}")
